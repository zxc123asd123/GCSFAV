...
def SMT_Solver(init_ego, dst_ego, time_ego, dst_p, region_id, behavior):
    ...
    init_c = [
        If(region_id in [1, 2], If(behavior == 'Pedestrian Cross', init_y == -41, init_y == -39),
                 ...
                 If(region_id in [8, 9], And(15 <= init_x, init_x <= 50),
                    If(region_id in [6, 11], Or(init_x == -8, init_x == -4),
                       If(region_id in [7, 10], Or(init_x == 0, init_x == 4),
                          If(region_id == 12, And(-8 <= init_x, init_x <= 4), And(-60 <= init_x, init_x <= -50))))))))
    ]
           ...
           If(behavior == 'Follow Lane', And(adjust_wp_x[0] == -21, adjust_wp_y[0] == init_y),
              If(behavior == 'Change Lane', If(region_id == 4, And(adjust_wp_y[0] == init_y - 4, adjust_wp_x[0] == -21),
                 ...
                 If(behavior == 'Cut In', And(adjust_wp_y[0] == init_y - 4, adjust_wp_x[0] == -21),
                    If(behavior == 'Vehicle Cross', And(adjust_wp_x[0] == init_x, adjust_wp_y[0] == -50),
                       If(behavior == 'Turn Around',
                          ...
                          If(behavior == 'Retrograde',
                             ...
                             If(behavior == 'Pedestrian Cross', And(adjust_wp_y[0] == init_y, adjust_wp_x[0] == -20),
                                And(adjust_wp_x[0] == init_x, adjust_wp_y[0] == init_y))))))))),
        If(behavior == 'Pedestrian Walk', And(adjust_wp_x[1] == init_x, adjust_wp_y[1] == init_y),
           If(And(behavior == 'Change Lane', region_id == 8),
              And(And(dst_ego[0] <= adjust_wp_x[1], adjust_wp_x[1] <= 14), adjust_wp_y[1] == -33),
              And(If(region_id == 9, adjust_wp_x[1] == 14, adjust_wp_x[1] == 4), adjust_wp_y[1] == dst_ego[1])))
    ]
